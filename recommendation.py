...
ROOT_DIR = 'Record Collections\Refined Medical Record'
TARGET_FILE = 'Record Collections\ConsineHashMap.csv'

# 加载停用词表
def load_stopwords():
    stopwords = []
    file_name = os.path.join('Record Collections', 'hit_stopwords.txt')
...
def main():
    ...
    for i in range(len(df)):
        hashmap[df.iloc[i, 0]] = df.iloc[i, 1:].dropna().tolist()
    ...
    while True:
        ...
        if filename in hashmap:
            ...
            print([filename_list[index].split('\\')[-1].split('.txt')[0] for index in top_k_index])
